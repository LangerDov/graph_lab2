...
void WindowThread(void* data)
{
    ...
    {
       mvp = Matrix::Perspective(3.f / 3.f, 45.f, 0.1, 100.f) *
            Matrix::Camera(Vector(0.f, 0.f, -1.f), Vector(0.f, 0.f, 5.f)) *
            Matrix::Translate(Vector(0, x, z)) *
            Matrix::RotateByAxis(Vector(0.f, 1.f, 1.f), angle);
        pRender->StartQueue();

        pPipeline->Bind(RENDERMODE_FILL);
        pPipeline->PushConstant((void*)mvp.Array(), sizeof(Matrix));
        pPipeline->DrawIndexed(pGpuRegion, 0, sizeof(vertices), 3);
